...
Device.prototype.getService= function getService(types, callback) {
  ...
  this._getXml(this.description, function(err, info) {
    ...
    // Work-around newer versions of xml2js
    if (info.root)
      info = info.root;
    ...
    if (s.length === 0 || !s[0].controlURL[0] || !s[0].SCPDURL[0]) {
      return callback(Error('Service not found'));
    }

    var base = url.parse(info.URLBase[0] || self.description);
    function prefix(u) {
      var uri = url.parse(u);
...
Device.prototype.parseDescription = function parseDescription(info) {
  ...
  function traverseDevices(device) {
    ...
    if (device.serviceList && Array.isArray(device.serviceList)) {
      for (let i = 0, len = device.serviceList.length; i < len; i++) {
        device.serviceList[i].service.forEach(traverseServices);
      }
    }

    if (device.deviceList && Array.isArray(device.deviceList)) {
      for (let i = 0, len = device.deviceList.length; i < len; i++) {
        device.deviceList[i].device.forEach(traverseDevices);
      }
    }
  }

  toArray(info.device).forEach(traverseDevices);
  ...
  this.getService(this.services, function(err, info) {
    ...
    request({
      ...
      parser.parseString(body, function(err, body) {
        ...
        callback(null, body[soapns + 'Envelope'][soapns + 'Body']);
      });
    });
  });
};
