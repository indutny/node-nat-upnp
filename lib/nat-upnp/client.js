  ...
  this.findGateway(function(err, gateway, address) {
    ...
    async.whilst(function() {
      ...
      gateway.run('GetGenericPortMappingEntry', [
        ...
        var key;
        Object.keys(data[0]).some(function(k) {
        ...
        var result = {
          public: {
            host: typeof data.NewRemoteHost[0] === 'string' &&
                  data.NewRemoteHost[0] || '',
            port: parseInt(data.NewExternalPort[0], 10)
          },
          private: {
            host: data.NewInternalClient[0],
            port: parseInt(data.NewInternalPort[0], 10)
          },
          protocol: data.NewProtocol[0].toLowerCase(),
          enabled: data.NewEnabled[0] === '1',
          description: data.NewPortMappingDescription[0],
          ttl: parseInt(data.NewLeaseDuration[0], 10)
        };
...
Client.prototype.externalIp = function externalIp(callback) {
  this.findGateway(function(err, gateway/*, address*/) {
    ...
    gateway.run('GetExternalIPAddress', [], function(err, data) {
      ...
      callback(null, data[0][key][0].NewExternalIPAddress[0]);
    });
  });
};
